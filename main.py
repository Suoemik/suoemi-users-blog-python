...
class User(UserMixin, db.Model):
    ...
    posts   = relationship("BlogPost", order_by=BlogPost.id, back_populates="author")
    comments    = relationship("Comment", order_by=Comment.id, back_populates="author")
...
def register():
    ...
    if request.method == "POST" and reg_form.validate_on_submit():
        found_user = User.query.filter_by(email=reg_form.email.data).first()
...
def login():
    ...
    if request.method == "POST" and log_form.validate_on_submit():
        formemail   = log_form.email.data
        formpass    = log_form.password.data
        found_user  = User.query.filter_by(email=formemail).first()

        if found_user:
            print("User found!")

            if check_password_hash(pwhash=found_user.password, password=formpass):
